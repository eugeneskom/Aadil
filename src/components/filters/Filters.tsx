...
import { fetchProductsAsync, selectMaxPrice, selectMinPrice } from "../../state/products/productsSlice";
...
function Filters() {
  ...
  return (
    <section className="filters relative z-10 "  ref={filterTabRef}>
        ...
        <ul className="flex flex-wrap -mx-2">
          <li className="px-2">
            <button
              ...
              name="price"
              className="flex items-center gap-1 border border-solid border-gray-200 text-gray-700 px-4 py-2 rounded-full hover:bg-gray-100 transition duration-300"
            >
      ...
      {activeTab && (
        <div className="absolute bg-white p-6 py-12  w-full">
          <FilterTab
            activeTab={activeTab}
            minPrice={minPrice}
            maxPrice={maxPrice}
